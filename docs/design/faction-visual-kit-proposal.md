...
Status: **accepted framework, phased. Phase 1 (fleet-wide palettes) implemented 2026-08-17; Phase 2 geometry completed and playtested 2026-08-18** — `data/visuals/faction_liveries.yaml` + `ShipVisualBuilder.LIVERY_MATERIAL_ROLES` recolor the GLB material slots at runtime (paint / accent / hull_wood / trim / streamer / sails roles); pirates got their own palette (2026-08-17, user request): tarred timber + dark oxblood band + blackened iron instead of gold + blood-red streamers + darker canvas than any nation — "absence of polish" as the recognition language; the class-brief schemes remain the fallback for unknown factions; `tools/_LiveryProbe.tscn` runs the flags-off recognition lineup. Pirate conversion kit **deferred by user directive (2026-08-17)**. Phase 2 shipped only where palette was not enough: `frigate_france.glb` / `FrenchFrigateVisual.tscn` carry the French graceful-geometry kit, and `brig_dutch.glb` / `DutchBrigVisual.tscn` carry the Dutch broader practical/deck-cargo kit. Spain and England remain palette-led after the flags-off/playtest read held.
...
**Phase 2 result (closed 2026-08-18).** The amended pilot order held: France received baked generator geometry on the frigate, Dutch received baked generator geometry on the brig, and the focused flags-off probe plus user playtest signed off the result. Do not expand national geometry further unless a new flags-off readability failure appears; the active visual backlog is now pirate conversion, upgrade overlays, and per-ship variants.

**Upgrade visuals (noted 2026-08-17, unscheduled).** Gameplay upgrades that read on the hull (e.g. copper bottoms at the waterline) layer into this system in three cost tiers: (1) *material-level* — cheap; the livery machinery (per-instance surface overrides, `apply_visuals` already receives `modifications`) plus, for copper, a small hull shader showing copper below a ship-local waterline measured from mesh bounds (sail-tatter material precedent); (2) *hidden-by-default geometry in the GLB* toggled at runtime — moderate, per-class Blender work; same mechanism as the planned Dutch deck barrels; (3) *runtime-composed geometry* — costly, breaks the flattened-GLB model, avoid. Composition rules: upgrades apply after livery (copper shader takes the livery hull color as its above-the-line color), and upgrade cues stay spatially distinct from faction cues — waterline/deck for upgrades, bands/trim/sails for faction.
